...
\begin{center}
    \begin{tcolorbox}[enhanced, breakable,
        ...
        \begin{itemize}
            ...
            \item 这玩意可以让用户更专注与内容、而不是被错综复杂的版式耍的团团转。%抓了个虫
...
我们仅仅需要打开 \verb`TeXlive` 的相关网站—— \url{https://tug.org/texlive/}，然后选择
...
\begin{table}[h]
    \begin{tblr}{colspec = {X[-1]X}, width = \textwidth}\hline
        宏包名                & 备注                                                         \\\hline
        \texttt{unicode-math} & 使用 Unicode 数学字体，并且允许在数学环境中使用 Unicode 字符 \\
                              & 具体参考 \url{https://www.ctan.org/pkg/unicode-math}         \\
        \texttt{mathtools}    & 一个提供了相当多数学公式拓展的宏包，提供了一些有用的符号     \\
                              & （比如说长等号什么的）                                       \\
        \texttt{tikz}         & 著名绘图宏包                                                 \\
        \texttt{tcolorbox}    & 可以排出各种各样的神奇妙妙彩色盒子                           \\
        \texttt{tabularray}   & 一个比较新的但是非常厉害的表格宏包                           \\
        \texttt{fancyhdr}     & 可以方便修改页眉页脚的格式                                   \\
        \texttt{titlesec}     & 修改章节标题格式                                             \\\hline
    \end{tblr}
    \caption{一些数学宏包们}
\end{table}
...
“呃如果想看看这玩意的用处就去看文档咯——比如那个 \verb"tcolorbox" 就很有意思。” 宇佐见莲子回答说，“反正我在这说也没啥用，你看了就知道了。如果遇到问题也可以到网上咨询：\url{https://tex.stackexchange.com}。”
...
宇佐见莲子说得没错，这玩意确实排公式方便，也十分美观。但作为古老的引擎，\TeX / \LaTeX 的活力远不如现在的其他排版 / 字处理软件——Microsoft Word、Adobe In Design等等那么充沛，这或许是 \LaTeX 所见非所得的特性和陡峭的学习曲线所致，但在排公式、科技类论文 \LaTeX 的确无出其右者，对排版细节的把握 \TeX 也是达到了相当高的水准。
